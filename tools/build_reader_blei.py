...
SECTIONS = [
    {
        ...
        "narration": (
            "One of the core problems of modern statistics is to approximate "
            "difficult-to-compute probability densities. This is especially important in "
            "Bayesian statistics, which frames all inference about unknown quantities as a "
            "calculation about the posterior. This paper reviews variational inference, a "
            "method that approximates these densities through optimization. The idea is to "
            "posit a family of approximate densities, and then find the member of that family "
            "closest to the target posterior, where closeness is measured by "
            "Kullback-Leibler divergence. Variational inference thus turns the inference "
            "problem into an optimization problem. Compared with Markov chain Monte Carlo: "
            "M-C-M-C samples from the posterior and is asymptotically exact, but can be "
            "computationally intensive; variational inference tends to be faster and easier "
            "to scale, though it only finds a density close to the target. So variational "
            "inference suits large data sets and settings where you want to explore many "
            "models quickly, while M-C-M-C suits smaller data sets where you'll happily pay "
            "more for precise samples. "
            "One quick connection to your own notes: this framing is exactly your bridge "
            "note, alternative inference engines for latent variable models. Keep it in mind "
            "as you read; the paper is the long-form version of that idea."
        ),
    },
    {
        "id": "s21",
