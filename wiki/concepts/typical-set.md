...
## Geometric face
The same fact in continuous form: samples from a $d$-dimensional standard Gaussian land on a thin shell at radius $\approx\sqrt{d}$ (density × shell-volume peaks away from the mode), not at the mode $r=0$. The typical set is that shell. See the interactive explainer at `explainers/typical-sets.html`.
