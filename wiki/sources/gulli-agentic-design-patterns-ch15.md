---
abbrev: "ADP"
id: "pkis:source:gulli-agentic-design-patterns-ch15"
aliases: []
title: "[ADP Gullí] Ch. 15 — Inter-Agent Communication (A2A)"
authors: "Antonio Gullí"
year: 2025
type: book-chapter
domain: [deep-learning]
tags: []
drive_id: "1_ODFqJ10y3xF1JFUcyDGOZljFJhDaRop"
drive_path: "PKIS/sources/books/AgenticDesignPatterns.pdf"
parent_book: "[[gulli-agentic-design-patterns]]"
chapter: 15
status: unread
date_added: 2026-05-20
concepts:
  - "[[multi-agent-systems]]"
---

Agent-to-agent communication protocols enabling direct peer-to-peer coordination between agents independent of a central orchestrator.
