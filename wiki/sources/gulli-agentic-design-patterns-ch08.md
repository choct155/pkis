---
abbrev: "ADP"
id: "pkis:source:gulli-agentic-design-patterns-ch08"
aliases: []
title: "[ADP Gullí] Ch. 8 — Memory Management"
authors: "Antonio Gullí"
year: 2025
type: book-chapter
domain: [deep-learning]
tags: []
drive_id: "1_ODFqJ10y3xF1JFUcyDGOZljFJhDaRop"
drive_path: "PKIS/sources/books/AgenticDesignPatterns.pdf"
parent_book: "[[gulli-agentic-design-patterns]]"
chapter: 8
status: unread
date_added: 2026-05-20
concepts: []
---

Managing agent state across interactions via session state, in-context scratchpad, and persistent long-term memory services.
